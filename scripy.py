...
df=dfs[0]
col = ['index', 'retrived_at', 'province_id','city','state',
       'country','pollution_aqi','pollution_main','weather_tp',
       'weather_pr','weather_hu','weather_ws','weather_wd']
df.columns = col
df = df.drop([0])
df = df.drop(['index','weather_ws','weather_wd'], axis = 1)
